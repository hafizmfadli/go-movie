...
# Prompt to enter a password for the PostgreSQL netflix user (rather than hard-coding
# a password in this script).
read -p "Enter password for netflix DB user: " DB_PASSWORD
...
# Set up the netflix DB and create a user account with the password entered earlier.
sudo -i -u postgres psql -c "CREATE DATABASE netflix"
sudo -i -u postgres psql -d netflix -c "CREATE EXTENSION IF NOT EXISTS citext"
sudo -i -u postgres psql -d netflix -c "CREATE ROLE netflix WITH LOGIN PASSWORD '${DB_PASSWORD}'"

# Add a DSN for connecting to the netflix database to the system-wide environment
# variables in the /etc/environment file.
echo "NETFLIX_DB_DSN='postgres://netflix:${DB_PASSWORD}@localhost/netflix'" >> /etc/environment
